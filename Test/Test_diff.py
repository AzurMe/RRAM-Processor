...
from Model.diff_model import Differential_RRAM_Processor

class Test_RRAM_VMM_Processor:
    def batch_test(self, num_samples=100):
        """
        随机生成测试集并分析误差（适配差分RRAM处理器）
        """
        ...
        # 优化：只创建一次处理器实例（避免重复初始化）
        processor_instance = Differential_RRAM_Processor()

        for i in range(num_samples):
            # 1. 随机生成输入 (0~1 浮点数，和模型匹配)
            vec = np.random.randint(0, 256, 4)
            # 2. 随机生成权重 (-1 ~ 1 浮点数，适配模型的差分架构)
            mat = np.random.randint(-15,16,(4,4))  # 核心修正：替换0/1整数为-1~1浮点数
...
if __name__ == "__main__":
    # 实例化测试类
    test_processor = Test_RRAM_VMM_Processor()

    # 运行批量测试
    ideal_data, actual_data = test_processor.batch_test(1000)
    ...
    # 图2: 误差直方图
    plt.subplot(1, 2, 2)
    error_values = (actual_data - ideal_data) / 255 / 4 / 15 * 100
