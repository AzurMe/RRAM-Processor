...
class RRAM_VMM_Processor_4b_nonideal:
    def __init__(self, g_sigma=0.05, v_noise=0.005, dac_gain_err = 0.01, n_adc = 0.005):
        ...
        # --- 4. LUT  ---
        # In actual chips, LUTs are used to eliminate HRS leakage and non-linear errors, mapping ADC codes back to logical results.
        self.calibration_value = 8
        self.lut = self.calibrate_lut()

        # --- Non-ideal parameters ---
        self.g_sigma = g_sigma  # Standard deviation of conductance fluctuations (e.g., 5%)
        self.v_noise = v_noise  # TIA thermal noise standard deviation (e.g., 5mV)
        self.n_adc = n_adc  # ADC input referred noise standard deviation (e.g., 5mV)
        self.dac_gain_err = dac_gain_err  # DAC gain error (1%)

    def calibrate_lut(self):
        lut = {}
        # Calculation: voltage contribution generated when input is 1 (digital) and weight is 1 (LRS)
    ...
    def rram_array_processing(self, voltage_vector, weight_matrix):
        """Calculate the equivalent conductance generated by a 4-bit weight (including 4 parallel units)"""
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                for bit_pos in range(4):
                    ...
                    # Simulated branch parallel: 1 corresponds to LRS, 0 corresponds to HRS
